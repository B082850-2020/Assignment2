...
def similarity(protein,taxon,partial):
	...
	if os.path.isfile(file_name + ".nuc.fa"):
				...
				if yes_no(within):
					...
					if yes_no(direc):
						try:	# try making a new directory 
							...
							subprocess.check_output(mkdir,shell=True)	# if making directory failed, try fails, except would proceed
		...
		# if there is only one species but not only one sequence do following
		elif int(count) > 1 and len(spe) == 1:
			...
			print("\n Only one sequence available, multiple alignment cannot be done based on current dataset. \n")
...
## profile function to characterizing putative structure and function of a sequence
# use the alignment output from similarity analysis 		
def profile(protein,taxon,partial):
	# to find the file generated by 'search' function
	file_name = ''.join(i for i in taxon if i.isalnum())
	# only carry on if the file exists
	if os.path.isfile(file_name + ".nuc.fa"):
		...
		if os.path.isfile(file_1 + ".align.fasta"):
			...
			profile = input("\n Profile analysis is a sequence comparison method for finding and aligning distantly related sequences.\n This helps characterizing putative structure and function of a sequence.\n\n Do you want to creat profiles for multiple aligned sequences? \n\n Please respond yes or no. ")
			# yes_no function: proceed if user responds yes
			if yes_no(profile):
		...
		if os.path.isfile(file_name + ".align.nuc.fa"):
			# print information about the analysis and provide choice to continue ot not 
			profile = input("\n Profile analysis is a sequence comparison method for finding and aligning distantly related sequences. This helps characterizing putative structure and function of a sequence.\n Do you want to creat profiles for multiple aligned sequences? \n\n Please respond yes or no. ")
			# yes_no function: proceed if user responds yes			
			if yes_no(profile):
				# shell command for creating prophecy file				
				prophecy = "prophecy -sequence " +  file_name + ".align.nuc.fa -type G -datafile Epprofile -name mymatrix -open 3.0 -extension 0.3 -outfile " + file_name + ".prophecy"
				subprocess.call(prophecy,shell=True)
				# shell command for creating prophet file				
				prophet = "prophet -sequence " + file_name + ".align.nuc.fa -infile " + file_name + ".prophecy -gapopen 1.0 -gapextend 1.0 -outfile " + file_name + ".prophet"
				subprocess.call(prophet,shell=True)
				print("\n Profiles created! .prophet files can be found under taxon name. \n")  
			# if user responds no, print message and end the script
			else:
				print("\n\n Thank you for using this programme! Bye! \n\n")
				quit() 
...
# call 'profile' function and pass multiple arguments from the 'details' library to the function as a list 
profile(*list(details.values()))
